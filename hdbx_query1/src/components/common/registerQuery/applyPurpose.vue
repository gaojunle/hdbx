<template>
  <div class="apply-purpose">
    <div class="title">选择您需要查询的用途</div>
    <ul class="clearfix">
...
import '@css/common/registerQuery/applyPurpose'
import hdRadio from '@share/components/hdRadio'
import { applyPurposes } from '@share/constants/queryOupsSoft'
...
export default {
  ...
  methods: {
    ...
    handleApplyType(value) {
      let item = this.applyPurposes.find(item => item.value === value)
      this.$emit('change', item)
      item.name === '其他' ? this.showTextArea() : this.hideTextArea()
    },
    /**
     * 显示文本框
     */
    ...
    setData(item) {
      ...
      if(item.name === '其他') this.showTextArea()
    },
  },
  mounted() {
  },
  ...
  created() {
    this.setData(this.applyPurpose)
  }
}
