  ...
  <div class="handle-mode">
    ...
    <ul class="handle-type-child" v-show="handleMode === 'xc'">
      <li v-for="(item, index) in handleModeChildren" :key="index">
        <hd-radio v-model="handleModeChild" :label="item.value" :name="'handleTypeChild'" @change="handleHandleModeChildren" v-html="item.name">
...
import '@css/common/registerQuery/handleMode'
...
export default {
  ...
  methods: {
    ...
    handleHandleMode(value) {
      ...
      if (this.handleMode === 'xc') value = 'tq'
