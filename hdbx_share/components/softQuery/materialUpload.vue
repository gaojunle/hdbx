  ...
  <div class="material-upload">
    ...
    <ul>
      <li v-for="(value, key) in uploadObj" :key="key">
        ...
        <file-upload :ref="key" :who="key" class="left" :showFont="value.showFont" @on-change="onChange" @on-complete="onComplete" @on-close="onClose"></file-upload>
  ...
  methods: {
    ...
    upload() {
      Object.keys(this.paramData).forEach(key => {
        this.params[key] = this.paramData[key]
      })
    ...
    setParams(data) {
      Object.keys(data).forEach((key, index) => {
        ...
        this.uploadObj['upload' + num].url = true
        this.$refs['upload' + num][0].initFile(data[key])
      })
    },
  },
  mounted() {
